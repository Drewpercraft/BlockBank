...
public class Branch {
	...
	public boolean isOutOfOrder() {
		...
		plugin.getLogger().info(
				"Checking out of order status for " + getTitle());
		if (getOpenHour() == 0 && getCloseHour() == 24) {
			...
			if (!outOfOrder) {
				...
				plugin.getLogger().fine(
						"Out of Order: Chance=" + chance + " / Fail Rate: "
								+ plugin.getATMFailRate());
				if (outOfOrder) {
					plugin.getLogger()
							.info(getTitle() + " is now out of order");
					BukkitTask task = new FixATMTask(plugin, bank.getName(),
							name).runTaskLater(plugin,
							plugin.getATMOfflineTime());
				}
			}
			return outOfOrder;
		}
	...
	/**
	 * @param state
	 *            set the failed status of this branch
	 */
	...
	public void loadConfiguration() {
		...
		ConfigurationSection branches = bank.getConfig()
				.getConfigurationSection("branches");
		if (branches == null) {
			branches = bank.getConfig().createSection("branches");
		}
		config = branches.getConfigurationSection(name);
		if (config == null) {
			config = bank.getConfig().getConfigurationSection("branches")
					.createSection(name);
			...
			config.set("transactionFee", bank.getConfig().getDouble("transactionFee", 0.0));
			plugin.saveConfig();
		}
	}
	...
	public String toString() {
		return String
				.format("%s branch of %s", config.getString("title"), bank);
	}
	...
	/**
	 * @param title
	 *            the display name of this branch
	 */
	public void setTitle(String title) {
		// TODO Check to make sure we're not renaming this bank to a name
		// already in use
	...
	public boolean isAnnouncements() {
		return this.config.getBoolean("announcements",
				this.plugin.getDefaultAnnouncements());
	}
	...
	public int getCloseHour() {
		return this.config.getInt("closeHour",
				this.plugin.getDefaultCloseHour());
	}

	/**
	 * @param closeHour
	 *            the closeHour to set
	 */
	...
	public double getTransactionFee() {
		return this.config.getDouble("transactionFee", 0.0);
	}
	
	public boolean setTransactionFee(double fee) {
		if (fee < 0 || fee > this.plugin.getMaxTransactionFee()) return false;
		this.config.set("transactionFee", fee);
		this.plugin.saveConfig();
		return true;
	}
	...
	public boolean isOpen() {
		World world = plugin.getServer().getWorld(
				this.config.getString("world"));
		int hour = Utils.GetWorldHour(world.getTime());
		return (hour >= this.config.getInt("openHour") && (hour < this.config
				.getInt("closeHour")));
	}
